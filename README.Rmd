...
```{r ex2_m3_summary, eval = TRUE}
predict(m, newdata = data.frame(pop = factor(levels(m$model$pop), levels(m$model$pop))), se.fit = TRUE)
```

And visualize using the built-in `plot.gamlss2`:

```{r ex2_m3_plot, eval = TRUE}
plot(m)
```
...
```{r ex3_m_summary, eval = TRUE}
summary(m)  # summary of the fit (intercept and slope)
```

```{r ex3_m_diagnostics, eval = TRUE}
plot(m)  # built-in diagnostics
```

```{r ex3_m_coefs, eval = TRUE}
...
```{r ibts_repl_gamlss, eval = FALSE}
# fit model wrt Year
# binning=TRUE to speedup fitting
m_gam_year = gamlss2(bodyMass ~ s(Year, bs = "re"), 
                data = dat, 
                family = bPL(dat$Number, dat$mn, dat$mx), 
                control = gamlss2_control(binning = TRUE)) 

# fit, also, a model accounting for a simple inter-species effect on 'b'
m_gam_year_species = gamlss2(bodyMass ~ s(Year, bs = "re") +  s(SpecCode, bs = "re"), 
                             data = dat, 
                             family = bPL(dat$Number, dat$mn, dat$mx), 
                             control = gamlss2_control(binning = TRUE))

# collect predicted effects from both models (in a similar notation to `sizeSpectra`)
...
Also fit the `brm` model:
```{r ibts_repl_brm, eval = FALSE}
# using the truncated Pareto parameterized with Stan in the `isdbayes` package
...
# collect the effects of the `brm` fit (in a similar notation to `sizeSpectra`)
...
rbind(cbind(method = "gamlss - only year", eff_gam_year[, c("Year", "b", "confMin", "confMax")]), 
  ...
  ggplot() + 
  theme_classic(base_size = 15) + 
  geom_point(aes(x = Year, y = b, colour = method), position = position_dodge(width = 0.25)) + 
  geom_errorbar(aes(x = Year, ymin = confMin, ymax = confMax, colour = method), width = 0.0, position = position_dodge(width = 0.25))
```


Besides this simple random year intercept models, we can, also, fit a more complex model using the `IBTS_data` dataset. First clean-up the data a bit:
```{r ibts_new, eval = FALSE}
...
```{r ex4_m, eval = FALSE}
# binning=TRUE for faster fitting
